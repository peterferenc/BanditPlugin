...
| `/banditevent <cost> [<metres>\|marker] [team:<team>] [seed:<n>]`<br>(aliases `/event`, `/bevent`) | Spawns a whole fight bought against a points budget - squads and crewed vehicles - down your sightline or at your map marker. The number is a cost, not a difficulty: see "Points and cost" below. `seed:` reproduces one event exactly; the reply prints the seed it used. |
...
| `/banditcost [apply]`<br>(aliases `/bcost`, `/costs`) | Suggests a points cost for every kit and vehicle from the game's own asset data, with the working shown. `apply` writes the suggestions into the configuration. See "Points and cost" below. |
...
| Command | Description |
|---|---|
| `/banditteam [list\|join <team>\|leave\|<player> <team>]`<br>(aliases `/team`, `/teams`, permission `bandit.team`) | Shows the teams, puts you or another player on one, or takes you off. Bandits never shoot their own team and always shoot the others. See "Teams" below. |
