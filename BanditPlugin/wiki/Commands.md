...
| `/banditevent convoy <cost> [vehicles:<n>] [crew:<n>] [useRoads:false] [team:<team>] [seed:<n>]` | Spawns a convoy: vehicles full of bandits, bought against the budget, at the first convoy waypoint and driving the rest of the route. Needs at least two waypoints from `/banditevent wp`. `useRoads:false` makes it drive straight lines instead of following the road graph. `vehicles:1 crew:1` is the one-vehicle, driver-only column - a route being driven and nothing else, which is the shape to test a route with. |
